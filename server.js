...
const naverAuth = require('./auth');   // ✅ 이 한 줄만 추가
...
app.use('/auth', naverAuth);   // ✅ 네이버 로그인 route 추가
...
app.post('/api/recommend-course', async (req, res) => {
  ...
  const prompt = `당신은 전문 한국어 여행 가이드입니다. 사용자의 MBTI는 ${mbti}이고 여행지는 ${region}입니다.
당신의 답변은 반드시 한국어로만 작성되어야 합니다.
사용자에게 ${region}에서 ${mbti} 성향에 딱 맞는 1일 여행 코스를 상세하고 친절하게 추천해 주세요.
코스는 다음 순서로 작성해 주세요: 1. 숙소 추천 🏨, 2. 카페 추천 ☕, 3. 산책 코스 추천 🌳.
중간중간 한국어 이모지도 활용해서 재미있게 알려주세요.`;
